...
new bool:g_bFirstJump[MAXPLAYERS+1] = {false, ...};
...
public OnPluginStart()
{
	...
	if(g_Settings[MultimodeEnable]){
		RegConsoleCmd("sm_style", Command_Difficulty);
		RegConsoleCmd("sm_bhop", Command_Difficulty);
		RegConsoleCmd("sm_mode", Command_Difficulty);
...
public Action:Event_PlayerJump(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(g_fStamina[client] != -1.0)
	{
		...
		//PrintToChat(client, "stamina override: %.1f", g_fStamina[client]);
	}
	//else PrintToChat(client, "no stamina override");
...
public MenuHandler_Physics(Handle:menu, MenuAction:action, client, itemNum)
{
	...
	else if (action == MenuAction_Cancel) 
	{
		...
		{
			if(IsClientInGame(client)) FakeClientCommand(client, "sm_bhop");
		}
	} 
...
AlterBhopBlocks(bool:bRevertChanges) 
{
	if(g_Settings[MultiBhopEnable])
	{
		...
		if(bRevertChanges) 
		{
			new doorcount, buttoncount;
			Timer_LogDebug("Start unregister bhop blocks");
			...
			Timer_LogDebug("Unregistered %d buttons and %d doors", buttoncount, doorcount);
		}
		else 
		{
			new doorcount, buttoncount;
			Timer_LogDebug("Start register bhop blocks");
			...
			Timer_LogDebug("Registered %d buttons and %d doors", buttoncount, doorcount);
		}
	}
}
